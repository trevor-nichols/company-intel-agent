...
Yyou can view a summary of the model's reasoning using the the `summary` parameter.

To access the most detailed summarizer available for a model, set the value of this parameter to `auto`. `auto` will be equivalent to `detailed` for most reasoning models today.

Reasoning summary output is part of the `summary` array in the `reasoning` [output item](/docs/api-reference/responses/object#responses/object-output).
